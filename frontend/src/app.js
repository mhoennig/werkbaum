...
import { ticketRefOf, taskCandidates, appendToken, refToken, slugToken, refParts, ticketUrl, ticketRefAt, ticketApiPath, mapTaigaStatus, taigaStatusName, pickStatus, statusApiPath, statusListPath, storyAncestor, collectTicketRefs, bulkPath, ticketDiverges, refVisibleInLabel, parseTicketInput, foreignTaigaUrl } from './taiga.js';
...
async function refreshTaiga(){
  ...
  if(!taigaProbeCache.has(basis)){
    taigaProbeCache.set(basis,
      fetchJson(live.infoUrl(basis))
        /* `taigaWeb` (D91-Nachtrag 5): die Web-Basis der Instanz für die
           Ticket-Links — fehlt sie, entfällt nur das Öffnen, nicht die Anlage. */
        .then(i => ({on: !!(i && i.taiga), web: (i && i.taigaWeb) || null}))
        .catch(() => ({on: false, web: null})));
  }
...
/* Die Taiga-Adresse des Knotens einer Zeile (D91-Nachtrag 5): Ref von der
   Zeile, Slug vom nächsten Vorfahren mit `&taiga.*`-Tag (SPEC §1), Web-Basis
   aus `GET /info`. null, wo eines davon fehlt — dann gibt es nichts zu
   öffnen, still (dieselbe Haltung wie beim Abhängigkeits-Sprung, D67). */
function taigaTicketUrl(lineNo){
  if(!taigaWeb) return null;
  const {node, roots} = nodeAndRootsByLine(lineNo);
  if(!node) return null;
  return ticketUrl(taigaWeb, taigaSlugs(roots).get(node), ticketRefOf(node));
}

function appendTaigaActions(el){
  /* Mit Taiga-Knöpfen darf das Fenster breiter werden (`tipwide`), damit die
     Aktionen NEBENEINANDER stehen; umgebrochen wird erst, wenn der Platz
     wirklich nicht reicht (flex-wrap). Die Klasse muss hier auch wieder
     heruntergenommen werden — das Fenster-Element wird wiederverwendet. */
  nodeTip.classList.remove('tipwide');
  if(!taigaOn) return;
  const line = +el.dataset.line;
  if(!line) return;
  const {node, roots} = nodeAndRootsByLine(line);
  if(!node) return;
  ...
  if(ref){
    /* Der Knoten IST schon angelegt (Idempotenz-Marker) — statt der
       Anlage-Knöpfe gibt es den Stand des Tickets, und in dessen
       Aktions-Zeile sitzt auch der ↗-Öffnen-Knopf (paintTicket): eine
       Zeile für alle Taiga-Knöpfe. Der Slug (SPEC §1) trägt das Lesen,
       die Web-Basis das Öffnen — fehlt eines, entfällt nur dessen Hälfte. */
    const slug = taigaSlugs(roots).get(node);
    if(slug){
      nodeTipBody.appendChild(ticketBox(el, slug, ref, line));
      nodeTip.classList.add('tipwide');
    }
    return;
  }
  ...
  wrap.appendChild(mk(t('taigaLinkBtn'), taigaLink));
...
function paintTicket(box, key, slug, ref, lineNo){
  ...
  if(st.kind === 'ok'){
    ...
    /* EINE Aktions-Zeile für alle Knöpfe — Schreiben/Übernehmen (paintDiff)
       und der ↗-Öffnen-Knopf nebeneinander; flex-wrap bricht erst um, wenn
       der Platz nicht reicht. Der Öffnen-Knopf ist auf Touch der einzige
       Weg zum Ticket (kein Strg) und macht die Strg+Klick-Geste
       auffindbar (D25-Lehre). */
    const act = document.createElement('div');
    act.className = 'tk-act';
    paintDiff(box, line, st, key, slug, ref, lineNo, act);
    const u = ticketUrl(taigaWeb, slug, ref);
    if(u) act.appendChild(openBtn(u, ref));
  ...
  const act = document.createElement('div');
  act.className = 'tk-act';
  act.appendChild(reloadBtn(key, slug, ref, st.kind === 'err'));
  const u = ticketUrl(taigaWeb, slug, ref);
  if(u) act.appendChild(openBtn(u, ref));
  box.appendChild(act);
}

/* Der ↗-Knopf zum Ticket im Taiga-Frontend (D91-Nachtrag 5). */
function openBtn(url, ref){
  return mkBtn('↗ ' + t('taigaOpenBtn', {ref: '#' + ref}),
    () => { closeNodeTip(); window.open(url, '_blank', 'noopener'); });
}
...
function paintDiff(box, line, st, key, slug, ref, lineNo, act){
  ...
  /* Schreiben geht nur für die fünf abgebildeten Zustände (D91-Nachtrag 7);
     ...
     fehlt Taigas Sperre; dann wird nicht angeboten. Die Knöpfe gehen in
     die GEMEINSAME Aktions-Zeile des Aufrufers (`act`). */
...
/* Strg+Klick (macOS auch Cmd) auf einen Knoten mit Ticket-Referenz öffnet
   ...
   Taiga-Adresse bleibt auch der Browser-Default (Strg+Klick auf einen
   Link-Knoten öffnet dessen URL im Hintergrund-Tab) unangetastet. */
out.addEventListener('click', e => {
  if(e.altKey || !(e.ctrlKey || e.metaKey)) return;
  const el = nodeFromEvent(e);
  if(!el || !el.dataset.line) return;
  const url = taigaTicketUrl(+el.dataset.line);
  if(!url) return;
  e.preventDefault();
  e.stopPropagation();
...
/* „Ticket verknüpfen" (D91-Nachtrag 11): ein BESTEHENDES Taiga-Ticket an den
   Knoten binden. Ein Feld nimmt die Taiga-URL — sie trägt Slug, Typ und
   Nummer selbst, die Projekt-Frage entfällt — oder eine Ref/Nummer; dann
   kommt das Projekt aus der Auswahl (vorbelegt aus dem geerbten
   `&taiga.<slug>`). Eine nackte Nummer wird per Probe aufgelöst (Story, dann
   Task — Taigas Zähler ist je Projekt gemeinsam, genau einer trifft).

   VOR dem Schreiben wird das Ticket geholt und der Betreff gezeigt:
   Verknüpft wird, was man gesehen hat, nicht eine blind eingetragene Nummer.
   Geschrieben wird wie bei der Anlage — Ref-Token plus ggf. `&taiga.<slug>`,
   ein Undo-Schritt —, und der geholte Stand füllt den Ticket-Cache gleich
   mit, sodass Fenster und Abweichungs-Marke ohne weiteren Abruf stimmen. */
async function taigaLink(line){
  const info = nodeAndRootsByLine(line);
  const node = info.node;
  if(!node || ticketRefOf(node)) return;
  const session = await taigaEnsureSession();
  ...
  const dlg = taigaModal(t('taigaLinkTitle'), t('taigaLinkDo'));
  const subject = document.createElement('p');
  subject.className = 'taiga-subject';
  subject.textContent = node.label;
  ...
  labIn.className = 'taiga-lab';
  const inSpan = document.createElement('span');
  inSpan.textContent = t('taigaLinkHint');
  labIn.append(inSpan, input);
  const sel = document.createElement('select');
  sel.disabled = true;
  const labSel = document.createElement('label');
  labSel.className = 'taiga-lab';
  const selSpan = document.createElement('span');
  selSpan.textContent = t('taigaProject');
  labSel.append(selSpan, sel);
  const preview = document.createElement('p');
  preview.className = 'taiga-subject';
  dlg.body.append(subject, labIn, labSel, preview);
  dlg.ok.disabled = true;
  dlg.cancel.addEventListener('click', () => dlg.close());
  ...
  /* Auflösen beim Tippen (400 ms Ruhe), nicht erst am Ok — die Bestätigung
     per Betreff soll DA sein, bevor man bestätigt. `seq` verwirft die
     Antwort eines überholten Abrufs (wer weitertippt, meint das Neuere). */
  let resolved = null, seq = 0, timer = null;
  const sag = msg => { dlg.err.textContent = msg; dlg.err.hidden = false; };
  const resolve = async () => {
    const my = ++seq;
    resolved = null;
    dlg.ok.disabled = true;
    dlg.err.hidden = true;
    preview.textContent = '';
    const p = parseTicketInput(input.value);
    if(!p){ sel.disabled = false; return; }
    if(foreignTaigaUrl(p, taigaWeb)){ sag(t('taigaLinkForeign')); return; }
    /* Eine URL trägt ihr Projekt selbst — die Auswahl tritt zurück. */
    sel.disabled = p.kind === 'url';
    const slug = p.kind === 'url' ? p.slug : (projects[sel.selectedIndex] || {}).slug;
    if(!slug) return;
    preview.textContent = t('taigaTicketLoading');
    try{
      let ref = p.ref, data;
      if(p.kind === 'nr'){
        try{
          ref = 'US-' + p.nr;
          data = await taigaFetch(ticketApiPath(ref, slug), null, session.token);
        }catch(e1){
          if(!e1 || e1.status !== 404) throw e1;
          ref = 'T-' + p.nr;
          data = await taigaFetch(ticketApiPath(ref, slug), null, session.token);
        }
      }else{
      ...
      if(my !== seq) return;
      resolved = {slug, ref, data};
      preview.textContent = '#' + ref + ' · ' + (data.subject || '');
      dlg.ok.disabled = false;
    }catch(err){
      if(my !== seq) return;
      preview.textContent = '';
      if(err && err.status === 404){
        sag(t('taigaLinkNotFound', {ref: '#' + (p.kind === 'nr' ? p.nr : p.ref), slug}));
  ...
  input.addEventListener('input', () => { clearTimeout(timer); timer = setTimeout(resolve, 400); });
  sel.addEventListener('change', resolve);
  dlg.ok.addEventListener('click', () => {
    if(!resolved) return;
    const p = refParts(resolved.ref);
    /* Dieselbe Schlagwort-Regel wie beim Anlegen (D91-Nachtrag 3/4). */
    const ownTag = (node.marks || []).some(m => m.startsWith('taiga.'));
    const tokens = (!ownTag && inherited !== resolved.slug) ? [slugToken(resolved.slug)] : [];
    writeLineTokens(node.line, tokens.concat(refToken(p.kind, p.nr)));
    taigaTickets.set(resolved.slug + '/' + resolved.ref, {kind: 'ok', data: resolved.data});
    dlg.close();
  });
  input.focus();
}
...
/* … und auf eine FREISTEHENDE Ticket-Referenz (`#US-123`/`#T-1234`, SPEC
   §11) öffnet dieselbe Geste das Ticket im Taiga-Frontend (D91-Nachtrag 5) —
   sofern das Backend die Web-Basis meldet (`taigaWeb`) und der Teilbaum
   einem `&taiga.*`-Projekt zugeordnet ist. Die Abhängigkeits-Lesart gewinnt:
   In einem `:#…`-Token ist die Ref nicht freistehend, dort bleibt es beim
   Sprung zur Zeile. */
function openTicketAtCaret(){
  const hit = ticketRefAt(src.value, src.selectionStart);
  if(!hit || !taigaWeb) return false;
  const {node, roots} = nodeAndRootsByLine(hit.line);
  if(!node) return false;
  const url = ticketUrl(taigaWeb, taigaSlugs(roots).get(node), hit.ref);
  if(!url) return false;
  window.open(url, '_blank', 'noopener');
  return true;
}
src.addEventListener('click', e => {
  if(e.altKey || !(e.ctrlKey || e.metaKey)) return;
  if(!jumpToDepTarget()) openTicketAtCaret();
});
src.addEventListener('keydown', e => {
  if(e.key !== 'Enter' || !(e.ctrlKey || e.metaKey) || e.altKey) return;
  if(jumpToDepTarget() || openTicketAtCaret()) e.preventDefault();
});
...
const I18N = {
  de: {
    ...
    tabConflictWarn:"„{name}“ wird in einem anderen Fenster ({where}) bearbeitet — die zuletzt gespeicherte Änderung gewinnt.",
    ...
    docLockText:"Ein anderes Fenster bearbeitet dieses Dokument gerade — die zuletzt gespeicherte Änderung gewinnt, auch das Falten im Diagramm. Was willst du tun?",
  ...
  en: {
    ...
    tabConflictWarn:"“{name}” is being edited in another window ({where}) — whichever change is saved last wins.",
    ...
    docLockText:"Another window is editing this document right now — whichever change is saved last wins, folding in the diagram included. What would you like to do?",
  ...
  es: {
    ...
    taigaStoryBtn:"Taiga: crear historia", taigaTaskBtn:"Taiga: crear tarea", taigaTaskNoProject:"Sin acceso al proyecto «{slug}».", taigaLoginTitle:"Iniciar sesión en Taiga", taigaUser:"Usuario", taigaPass:"Contraseña", taigaLoginDo:"Iniciar sesión", taigaCancel:"Cancelar", taigaProjectTitle:"Crear ticket de Taiga", taigaProject:"Proyecto", taigaTasksPick:"Crear subpaquetes como tareas:", taigaCreateDo:"Crear", taigaError:"Error: {error}", taigaOpenBtn:"Abrir {ref}", taigaLinkBtn:"Taiga: vincular ticket", taigaLinkTitle:"Vincular ticket de Taiga", taigaLinkDo:"Vincular", taigaLinkHint:"URL de Taiga o ref (US-123, T-45, 123)", taigaLinkNotFound:"No hay ticket {ref} en el proyecto «{slug}».", taigaLinkForeign:"La dirección pertenece a otra instancia de Taiga.",taigaTicketDiff:"difiere del plan:", taigaTicketNoBox:"sin casilla de estado", taigaTicketPush:"escribir en Taiga", taigaTicketPull:"tomar de Taiga", taigaTicketWriting:"escribiendo …", taigaTicketNoMap:"Este estado no tiene equivalente en Taiga: no se escribe nada.", taigaTicketNoColumn:"En este proyecto no existe la columna «{name}».", taigaTicketFetch:"Consultar estado", taigaTicketReload:"Volver a consultar", taigaTicketLoading:"Consultando estado …", taigaTicketAssignee:"Asignado a: {name}",
    ...
    tabConflictWarn:"«{name}» se está editando en otra ventana ({where}) — gana el último cambio guardado.",
    ...
    docLockText:"Otra ventana está editando este documento ahora mismo — gana el último cambio guardado, incluido el plegado en el diagrama. ¿Qué quieres hacer?",
  ...
  fr: {
    ...
    taigaStoryBtn:"Taiga : créer la story", taigaTaskBtn:"Taiga : créer une tâche", taigaTaskNoProject:"Pas d'accès au projet « {slug} ».", taigaLoginTitle:"Se connecter à Taiga", taigaUser:"Nom d'utilisateur", taigaPass:"Mot de passe", taigaLoginDo:"Se connecter", taigaCancel:"Annuler", taigaProjectTitle:"Créer un ticket Taiga", taigaProject:"Projet", taigaTasksPick:"Créer les sous-lots comme tâches :", taigaCreateDo:"Créer", taigaError:"Échec : {error}", taigaOpenBtn:"Ouvrir {ref}", taigaLinkBtn:"Taiga : lier un ticket", taigaLinkTitle:"Lier un ticket Taiga", taigaLinkDo:"Lier", taigaLinkHint:"URL Taiga ou réf (US-123, T-45, 123)", taigaLinkNotFound:"Pas de ticket {ref} dans le projet « {slug} ».", taigaLinkForeign:"L'adresse appartient à une autre instance Taiga.",taigaTicketDiff:"diffère du plan :", taigaTicketNoBox:"sans case d'état", taigaTicketPush:"écrire dans Taiga", taigaTicketPull:"reprendre de Taiga", taigaTicketWriting:"écriture …", taigaTicketNoMap:"Cet état n'a pas d'équivalent dans Taiga — rien n'est écrit.", taigaTicketNoColumn:"La colonne « {name} » n'existe pas dans ce projet.", taigaTicketFetch:"Relever l'état", taigaTicketReload:"Relever à nouveau", taigaTicketLoading:"Relevé de l'état …", taigaTicketAssignee:"Assigné à : {name}",
    ...
    tabConflictWarn:"« {name} » est modifié dans une autre fenêtre ({where}) — la dernière modification enregistrée gagne.",
    ...
    docLockText:"Une autre fenêtre modifie ce document en ce moment — la dernière modification enregistrée gagne, y compris le pliage dans le diagramme. Que veux-tu faire ?",
  ...
  pl: {
    ...
    tabConflictWarn:"„{name}” jest edytowany w innym oknie ({where}) — wygrywa ostatnia zapisana zmiana.",
    ...
    docLockText:"Inne okno właśnie edytuje ten dokument — wygrywa ostatnia zapisana zmiana, także zwijanie w diagramie. Co chcesz zrobić?",
  ...
  ru: {
    ...
    taigaStoryBtn:"Taiga: создать историю", taigaTaskBtn:"Taiga: создать задачу", taigaTaskNoProject:"Нет доступа к проекту «{slug}».", taigaLoginTitle:"Вход в Taiga", taigaUser:"Имя пользователя", taigaPass:"Пароль", taigaLoginDo:"Войти", taigaCancel:"Отмена", taigaProjectTitle:"Создать тикет в Taiga", taigaProject:"Проект", taigaTasksPick:"Создать подпакеты как задачи:", taigaCreateDo:"Создать", taigaError:"Не удалось: {error}", taigaOpenBtn:"Открыть {ref}", taigaLinkBtn:"Taiga: связать тикет", taigaLinkTitle:"Связать тикет Taiga", taigaLinkDo:"Связать", taigaLinkHint:"URL Taiga или реф (US-123, T-45, 123)", taigaLinkNotFound:"Тикета {ref} нет в проекте «{slug}».", taigaLinkForeign:"Адрес относится к другой инстанции Taiga.",taigaTicketDiff:"расходится с планом:", taigaTicketNoBox:"без статуса", taigaTicketPush:"записать в Taiga", taigaTicketPull:"взять из Taiga", taigaTicketWriting:"запись …", taigaTicketNoMap:"У этого состояния нет соответствия в Taiga — ничего не записано.", taigaTicketNoColumn:"В этом проекте нет колонки «{name}».", taigaTicketFetch:"Получить статус", taigaTicketReload:"Обновить статус", taigaTicketLoading:"Получение статуса …", taigaTicketAssignee:"Назначено: {name}",
    ...
    tabConflictWarn:"«{name}» редактируется в другом окне ({where}) — побеждает последнее сохранённое изменение.",
    ...
    docLockText:"Другое окно сейчас редактирует этот документ — побеждает последнее сохранённое изменение, включая сворачивание в диаграмме. Что вы хотите сделать?",
  ...
  hi: {
    ...
    taigaStoryBtn:"Taiga: स्टोरी बनाएँ", taigaTaskBtn:"Taiga: टास्क बनाएँ", taigaTaskNoProject:"प्रोजेक्ट “{slug}” तक पहुँच नहीं है।", taigaLoginTitle:"Taiga में साइन इन करें", taigaUser:"उपयोगकर्ता नाम", taigaPass:"पासवर्ड", taigaLoginDo:"साइन इन", taigaCancel:"रद्द करें", taigaProjectTitle:"Taiga टिकट बनाएँ", taigaProject:"प्रोजेक्ट", taigaTasksPick:"उप-पैकेज टास्क के रूप में बनाएँ:", taigaCreateDo:"बनाएँ", taigaError:"विफल: {error}", taigaOpenBtn:"{ref} खोलें", taigaLinkBtn:"Taiga: टिकट लिंक करें", taigaLinkTitle:"Taiga टिकट लिंक करें", taigaLinkDo:"लिंक करें", taigaLinkHint:"Taiga URL या रेफ़ (US-123, T-45, 123)", taigaLinkNotFound:"प्रोजेक्ट “{slug}” में टिकट {ref} नहीं है।", taigaLinkForeign:"यह पता किसी और Taiga इंस्टेंस का है।",taigaTicketDiff:"योजना से भिन्न:", taigaTicketNoBox:"स्थिति-बॉक्स नहीं", taigaTicketPush:"Taiga में लिखें", taigaTicketPull:"Taiga से लें", taigaTicketWriting:"लिखा जा रहा है …", taigaTicketNoMap:"इस स्थिति का Taiga में कोई समकक्ष नहीं है — कुछ नहीं लिखा जाता।", taigaTicketNoColumn:"इस प्रोजेक्ट में “{name}” कॉलम नहीं है।", taigaTicketFetch:"स्थिति लाएँ", taigaTicketReload:"फिर से लाएँ", taigaTicketLoading:"स्थिति लाई जा रही है …", taigaTicketAssignee:"ज़िम्मेदार: {name}",
    ...
    tabConflictWarn:"„{name}“ दूसरी विंडो में संपादित हो रहा है ({where}) — अंत में सहेजा गया बदलाव जीतता है।",
    ...
    docLockText:"दूसरी विंडो इस दस्तावेज़ को अभी संपादित कर रही है — अंत में सहेजा गया बदलाव जीतता है, आरेख में मोड़ना भी। आप क्या करना चाहेंगे?",
  ...
  zh: {
    ...
    tabConflictWarn:"“{name}”正在另一个窗口中编辑（{where}）——最后保存的更改生效。",
    ...
    docLockText:"另一个窗口正在编辑此文档——最后保存的更改生效，包括在图中折叠节点。您想怎么做？",
  ...
  ja: {
    ...
    tipClose:"閉じる",
    tipOpenLink:"リンクを開く",
    taigaStoryBtn:"Taiga: ストーリーを作成", taigaTaskBtn:"Taiga: タスクを作成", taigaTaskNoProject:"プロジェクト「{slug}」にアクセスできません。", taigaLoginTitle:"Taiga にログイン", taigaUser:"ユーザー名", taigaPass:"パスワード", taigaLoginDo:"ログイン", taigaCancel:"キャンセル", taigaProjectTitle:"Taiga チケットを作成", taigaProject:"プロジェクト", taigaTasksPick:"サブパッケージをタスクとして作成:", taigaCreateDo:"作成", taigaError:"失敗: {error}", taigaOpenBtn:"{ref} を開く", taigaLinkBtn:"Taiga: チケットを紐付け", taigaLinkTitle:"Taiga チケットを紐付け", taigaLinkDo:"紐付ける", taigaLinkHint:"Taiga の URL または参照番号 (US-123, T-45, 123)", taigaLinkNotFound:"プロジェクト「{slug}」にチケット {ref} はありません。", taigaLinkForeign:"このアドレスは別の Taiga インスタンスのものです。",taigaTicketDiff:"計画と食い違い:", taigaTicketNoBox:"ステータス欄なし", taigaTicketPush:"Taiga に書く", taigaTicketPull:"Taiga から取る", taigaTicketWriting:"書き込み中 …", taigaTicketNoMap:"この状態に対応する列が Taiga にありません — 何も書き込みません。", taigaTicketNoColumn:"このプロジェクトに「{name}」列はありません。", taigaTicketFetch:"状態を取得", taigaTicketReload:"再取得", taigaTicketLoading:"状態を取得中 …", taigaTicketAssignee:"担当: {name}",
    ...
    tabConflictWarn:"「{name}」は別のウィンドウで編集中です（{where}）— 最後に保存された変更が優先されます。",
    ...
    docLockText:"別のウィンドウがこのドキュメントを編集中です — 最後に保存された変更が優先されます（図の折りたたみも含む）。どうしますか？",
